...
\node (Txt_Glu) [left=5px of Gelu] {GeLU activation};
...
\node (txt_AHl) [left=10px of AHl_1] {Hidden State $H$};
		...
		\begin{figure}
			...
			\begin{tikzpicture}
				\begin{axis}[
					...
					\addplot[domain=-5:5, samples=100, ultra thick, blue]{max(0,x)};
					\addplot[domain=-5:5, samples=100, ultra thick, red]{0.5*(1+tanh(sqrt(2/pi)*(x+0.044715*x^3)))};
