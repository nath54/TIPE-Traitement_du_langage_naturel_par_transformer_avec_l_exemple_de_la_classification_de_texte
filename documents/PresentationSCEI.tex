...
\node[Block, align=center] (EPE)  [above=15px of Tok] {Embedding + \\Encodage Positionnel};
...
\begin{frame}{1.1 Vectorisation du texte : Tokenisation du texte}
...
\begin{frame}{1.1 Vectorisation du texte : Embeddings \& Encodage Positionnel}
...
%\node (Model) [right=15px of LayerNorm] {Model};
  ...
  \node [fit=(TokensT) (LayerNorm) (EmbeddingE) (PositionalP), draw=red, thick, inner sep=5pt, rounded corners] (box) {};
...
\node[Ablock] (Att) [above=15px of Einp] {Self-Attention Multi-Tête};
...
%Blocks
\node (Inp) {``Entrée''};
\node[Block] (BT) [right=25px of Inp] {Tokenizer\\BERT};
...
\begin{tikzpicture}[
DecBlock/.style = {rectangle, rounded corners, draw=blue!30, fill=blue!5, thick, dashed, minimum width=180px, minimum height=335px},
Block/.style =  {rectangle,  rounded corners, draw=black!100, fill =white!0, thick, minimum width = 80px},
...
% MultiHead Self-Attention Block
\node[Ablock] (Att) [above=15px of Einp] {Self-Attention Masquée Multi-tête};
\node[Block] (AN1) [above=15px of Att] {Somme \& Normalise};
...
\node[Ablock] (Catt) [above=15px of Aout] {Cross-Attention Multi-tête};
...
\begin{center}
	...
	\begin{minipage}{(\textwidth * 24  / 20)}
		...
		\begin{multicols*}{2}[]
			...
			% MultiHead Self-Attention Block
			\node[Ablock] (Att) [above=15px of Dinp] {Self-Attention Masquée Multi-Tête};
			\node[Block] (AN1) [above=15px of Att] {Somme \& Normalise};
